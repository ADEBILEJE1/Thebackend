...
# async def print_customer_receipt(
#     order_id: str,
#     request: Request,
#     current_user: dict = Depends(require_sales_staff)
# ):
...
#     # Efficiently fetch all options for the order's items
#     order_items = order.get("order_items", [])
#     if order_items:
#         item_ids = [item['id'] for item in order_items]
#         options_result = supabase_admin.table("order_item_options").select("*, product_options(name, price_modifier)").in_("order_item_id", item_ids).execute()
...
#     # Build the HTML for the items table
#     items_html = ""
#     for item in order_items:
...
#     await log_activity(
#         current_user["id"], current_user["email"], current_user["role"],
#         "print", "customer_receipt", order_id, {"order_number": order["order_number"]}, 
#         request
#     )
    ...
    for item in order_items:
        ...
        for option in item.get("options", []):
            ...
            if opt_details:
                price_mod = Decimal(opt_details.get("price_modifier", 0))
                # --- THIS IS THE FIX ---
                # Changed opt_details["name"] to a safe .get() to prevent an error
